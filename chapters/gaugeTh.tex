...
meaning that it is equal to $\hat R^\eta$ plus \emph{second order terms in $\eta$}: for arbitrary $\oid X, \oid Y \in A$, and ignoring the immersion $j$, 
...
\begin{enumerate}
    ...
    \item The second term contains both the Kinetic energy of the tau fields, and coupling of the $A$ and $\tilde A$ fields with the $\tau$ fields in terms quadratic in the $A$ fields, which give rise to mass terms for these $A$ fields in the Lagrangian.
...
\begin{eqnsplit}\label{equationTrivializationMatterLagrangian}
    \mathcal L_{matter}
      ...
      &\quad \quad \quad \quad \quad \quad + h^{a_1 a_2} h^E_{u_1 u_2} ((\phi_L(\tau) \mu)\comp \tilde \omega_i)_{a_1}^{u_1} ((\phi_L(\tau) \mu)\comp \tilde \omega_i)_{a_2}^{u_2}\\
      &= (a^*\phi(\nabla)\mu_i)_{\nu}^{u} (a^*\phi(\nabla)\mu_i)^{\nu}_{u} 
