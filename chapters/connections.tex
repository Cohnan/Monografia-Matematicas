...
\end{equation}
This is called the \emphtext{decomposition of the curvature form of $\hat \omega$ given the background connection $\tilde \omega$}.
\end{theorem}
\begin{proof}
TODO \todo{}.
\end{proof}
...
\begin{eqnsplit}
    ...
    &= \Omega^1(TU_i, U_i \times \alg g) \oplus_{C^\infty(U_i)} \Omega^1(U_i \times \alg g, U_i \times \alg g).
\end{eqnsplit}
where 
    \begin{itemize}
    ...
    \item $\epsilon = Id_L \in \Omega^1(U_i \times \alg g, U_i \times \alg g)$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Let $\{E_a\}_{a = 1, dots, n}$ be a basis for $\alg g$. Let us also denote by $E_a$ the corresponding constant functions in $C^\infty(U_i, \alg g) = \Omega^0(TU_i \times \alg g, U_i \times \alg g)$ for all $i \in I$. Recall the convention of not write the wedge product symbol if one of the factors is a a $0$-form, like $E_a$.

\begin{definition}
    Let $a, b \in 1, \dots, n$. \emphtext{The components of $A_i$, $\epsilon$ and $\tau_i$ with respect to the basis} 
        ...
        \item $(\tau_i)^a_b \in C^\infty(U_i)$ such that
            \begin{equation}
                \tau_i(E_b) = \sum_{a = 1}^n (\tau_i)^a_b E_a
            \end{equation}.
...
\begin{definition}
    Let $\omega \in \Omega(A, L)$ be an ordinary connection form with local trivialization $\omega_i = A_i - \epsilon$ over each $U_i$, and let $a = 1, \dots, n$. The local $1$-forms, 
    \begin{equation}
        \alg a^a := (A_i)^a - \epsilon^a \in \Omega^1(TU_i \times \alg g, U_i \times \alg g)
    \end{equation}
    are called \emph{the mixed local basis}
\end{definition}
