...
\end{equation*}
where $ \in C^\infty(S^2)$; recall from example \ref{exampleTransitionALphaFormsPk} that this suffices to determine completely the form over all of $S^2$ since $U_S$ covers all but one point of $S^2$.

Due to the fact that $\alpha^N_S(Im) = Im - k d\theta$, its local trivialization over $U_N$ is
\begin{eqnsplit}
     \hat \omega_N &= \hat \omega_S + kd\theta\\
