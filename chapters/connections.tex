...
\begin{proposition}
Let $\phi:A \to \alg D(E)$ be a representation. Then there is a bijective correspondence between $A$-connections $\hat \nabla^E$ on $E$ and $1$-forms $\hat \omega^E: \Omega^1(\alg D(E), \End(E))$ given by the relation:
\begin{equation}
    \hat \nabla^E_{\oid X} = \phi(\oid X) + j \comp \hat \omega^E(\oid X).
\end{equation}
Furthermore, the curvature of $\hat \nabla^E$ satisfies:\todo{have to define $\hat d_E$ from induced representation of $A$ on $End(E)$}
\begin{equation}
    \hat R^E = \hat d_E \hat \omega^E + \frac{1}{2} \hat \omega^E \wedge^\comp \hat \omega^E.
\end{equation}
\end{proposition}

For a produced connection $\hat \nabla^{E, \hat \omega}$ for the connection $1$-form $\hat \omega \in \Omega^1(A, L)$, the induced $1$-form is $\phi_L \comp \hat \omega$, and so:
\begin{equation}
    \hat \nabla^{E, \hat \omega}_{\oid X} = \phi(\oid X) + j \comp \phi_l \comp \hat \omega (\oid X)
\end{equation}