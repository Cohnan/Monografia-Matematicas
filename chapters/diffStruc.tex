...
Recall that the spherical coordinates $(\phi, \theta)$, although, strictly speaking, they are not manifold coordinates on the neighborhood $U_{SN}$, they are so on two different charts that cover $U_{SN}$, and so $\{\partial_\phi, \partial_\theta\} \subset \Gamma_{U_{SN}}(S^2)$ is a local frame of $S^2$ associated to coordinate functions and so, on $U_{SN}$, we may once again apply remark \ref{remarkLocalDecompositionOfFormsOnTransitiveLieAlgebroidsIfTrivializationOfM} to write any form on $TP^k/S^1$ as the wedge product of the scalar valued local $1$-forms
\begin{align}
...
\end{align}
where $\{Im, Jm, Km \}$ is the restriction to the respective neighborhood of the global forms $\{Im, Jm, Km\} \subset \Omega^1(S^2 \times Im \HH)$ dual to the global frame $\{i, j, k\} \in C^\infty(S^4, Im \HH)$.
\end{example}
...
\end{align*}
where we are denoting by $\alpha^i_j$ the map $:C^\infty(U_{ij}, V) \to C^\infty(U_{ij}, V)$, $\alpha^i_j(f)(m) := \alpha^i_{j, m}(f(m))$ for any $f \in C^\infty(U_{ij}, V)$, which can be expressed as
\begin{align}
