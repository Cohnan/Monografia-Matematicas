...
\begin{definition}
Let $E$ be a Lie algebra bundle over $M$ on which $A$ is represented. Define on $\Omega^\cdot(A, E)$ the binary operation
\begin{eqnsplit}
\wedge : \Omega^p(A, E) \times \Omega^q(A, E) &\to \Omega^{p+q}(A, E) \\
(\sect \omega \wedge \sect \eta)(\sectoid X_1, \dots, \sectoid X_{p+q}) := \frac{1}{p!q!} \sum_{\sigma \in S_{p+q}} (-1)^{\sigma} []
\end{eqnsplit}
\end{definition}
