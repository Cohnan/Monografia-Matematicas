...
All we need to check is that $\hat d_\phi \circ \hat d_\phi = 0$.\todo{}
\end{proof}
...
\end{eqnsplit}
for any real valued $p$-form $\omega$, $L$-valued $q$-form $\eta$ and any $\sectoid X_1, \dots, \sectoid X_{p+q} \in \Gamma(A)$. This is called \emph{the wedge product of scalar and $E$-valued forms on $A$}.
\end{definition}
...
\begin{proof}
From the definition of the wedge product, it is clear that, for any $p, q \in \ZZ_{\geq 0}$, $\Omega^p(A) \wedge \Omega^q(A, E) \subset \Omega^{p+q}(A, E)$.

All we need to prove is that Leibniz rule, since then if follows from proposition \ref{propositionEValuedFormsOnAIsDifferentialComplex} that $\hat d_\phi$ is a differential of degree $1$.

Let $\omega \in \Omega^p(A)$, $\eta \in \Omega^q(A, E)$ \todo{Leibniz}
...
\begin{proof}
Graded Leibniz is the only piece missing. Perhaps here I need the condition $\phi(A) \subset \alg D_\bullet (E)$? \todo{}
\end{proof}
...
The following important theorem relates the notions of vector valued connections on principal bundles, that give rise to principal connections and their curvatures, and the theory of forms on the Atiyah Lie algebroid associated to the principal bundle. To the proofs we refer to \cite{}.
