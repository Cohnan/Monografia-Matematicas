...
\begin{itemize}
    \item The differential graded module and algebra \rtext{$(\Omega^\bullet(TP/G), \hat d_{TS^3/S^1})$ is isomorphic to subset of $(\Omega^\bullet(TP), d)$ associated to the $R_*$-invariant  forms on $P$}.
    
    \item Similarly, the differential graded module and algebra \rtext{$(\Omega^\bullet(TP/G, P \times \alg g/G), \hat d)$ is isomorphic to subset of $(\Omega^\bullet(TP) \otimes \alg g, d)$ associated to the $(R_*, Ad)$-equivariant $\alg g$-valued forms on $P$}.
\end{itemize} 
\end{theorem}
...
\begin{example}
Let $A = TM$ given a manifold $M$. Then $(\Omega^\bullet (TM), \wedge, \hat d_{TM})$ is precisely the usual differential graded algebra and module $(\Omega(M), \wedge, d)$.
...
So, the building blocks of any local form over $U_{SN}$ will be the dual frame, which we will denote by $\{\underline{d\phi}, \underline{d\xi_1}, \underline{\xi_2}\}$. 
