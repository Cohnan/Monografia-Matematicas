...
\begin{align}
    \omega &: \underbrace{A|_U \otimes \cdots \otimes A|_U}_{\text{$n$ times}} \to E|_U, & \text{alternating, $\RR$-vector bundle map and}\\
...
\begin{example}
On the trivial vector bundle $M \times \bb R$, the associated space of differential forms on $A$ is denoted by $\lbtext{\lbtext{\Omega^\bullet(A)}}$, and it is called the \lbtext{space of scalar valued forms on $A$}. Notice that the module $\Omega^n(A) = \Gamma \bigwedge^n A^*$.
...
\end{equation}
for all $\sectoid X in \Gamma(A)$ and all $\mu_1, \mu_2 \in \Gamma(E)$.
Define on $\Omega^\bullet(A, E)$ the $C^\infty(M)$-bilinear operation, called \emph{the wedge product of algebra bundle-valued forms on $A$}
\begin{eqnsplit*}
...
Let $E$ be an algebra bundle over $M$ on which there is a representation $\phi: A \to \alg D(E)$ of $A$ which is compatible with the algebra product. \textit{Then, $(\Omega^\bullet(A, E), \wedge^\bullet, \hat d_\phi)$ is a differential graded algebra over $\RR$}.
\end{theorem}
...
Let the representation space $E$ be a LAB, with a representation compatible with the Lie algebra bracket. Then $(\Omega^\bullet(A, E),$ $\wedge^\bullet, \hat d_\phi)$ is a differential graded Lie algebra.
\end{theorem}
\begin{proof}
...
\begin{itemize}
    \item The differential graded module and algebra \textit{$(\Omega^\bullet(TP/G), \wedge, \hat d_{TP/G})$ is isomorphic to the subspace of $(\Omega^\bullet(TP), \wedge, d)$ associated to the $R_*$-inva- riant  forms on $P$}.
    
    \item Similarly, the differential graded module and algebra \textit{$(\Omega^\bullet(TP/G, P \times \alg g/G), \hat d)$ is isomorphic to the subspace of $(\Omega^\bullet(TP) \otimes \alg g, \wedge, d)$ associated to the $(R_*, Ad)$-equivariant $\alg g$-valued forms on $P$}.
\end{itemize} 
\end{theorem}
...
\noindent Since $L = S^3 \times i\RR / S^1$ has the global frame $\{i\}$, any section of $L$ can be written as $f\,i$ for some $f \in C^\infty(S^2)$. Hence, \textit{any $S^3 \times i\RR/S^1$-valued local form of $TS^3/S^1$ over $U_{SN}$ can be written as a wedge product of the forms $\{\cl{d\phi}, \cl{d\xi_1}, \cl{\xi_2}\}$, with coefficients in $C^\infty(S^2, i\RR)$}.
...
\begin{equation}
    \lbtext{\omega^{MP}_{}} := \frac{i}{2} (1 + \cos \phi) \cl{d \xi_1} + \frac{i}{2} (1- \cos\phi)\cl{d\xi_2} \in \Omega_{U_{SN}}^1(TS^3/S^1, S^3 \times i\RR/S^1).
\end{equation} This $1$-form is associated to the presence of a magnetic monopole on $3$-dimensional space.
...
% % \begin{remark}
% % %The previous isomorphism between $\Omega^\bullet(TM \times \alg g)$ and $\Omega^\bullet(TM) \otimes_{\bb R} \bigwedge^\bullet \alg g^*$ can, and will, be used to define a $C^\infty(M)$-linear multiplication $\lbtext{\wedge}$ on $\Omega^\bullet(TM) \otimes_{\bb R} \bigwedge^\bullet \alg g^*$ which is compatible with the gradings. 

% % The previous isomorphisms allows us to think of $\Omega^\bullet(TM) \equiv \Omega^\bullet(TM) \otimes 1$ and $C^\infty(M, \alg g) \equiv 1\otimes C^\infty(M, \alg g)$ as subsets of $\Omega^\bullet(TM \times \alg g)$.% Furthermore the $\Omega^\bullet(TM)$-multiplication on the latter differential graded module coincides with the wedge product within $\Omega^\bullet(TM \times \alg g)$. Abusing notation we may ignore the $~$'s of the mapping to simply state that, for $\alpha \in \Omega^r(TM)$ and $\beta \in C^\infty(M)\otimes_{\bb R} \bigwedge^s \alg g^*$, $r, s \in \ZZ_{\geq 0}$, \ptext{$\alpha \wedge \beta \in \Omega^{r+s}(TM \times \alg g)$}. 
% % \end{remark}
...
% \end{theorem}
% \begin{proof}
% Recall that for scalar-valued forms the representation on $M \times \bb R$ is simply the anchor $a$. First, let $\alpha \in \Omega^p(TM)$:
% \begin{align*}
...
\end{eqnsplit}
The previous equation further shows that under no representation $\phi$ is $C^\infty(M,$ $\bigwedge^\bullet \alg g^* \otimes V)$ closed under the differential, due to the first part of the left hand side, and that $\Omega^\bullet(TM)\otimes V$ is closed under $\hat d$ if and only if $B$ and $\phi_L$ are $0$, i.e. $\phi = a$ is the trivial representation. This contrasts what is said in the guiding articles \cite{Lazzarini2012, Fournel2011, Fournel2013}.
\end{remark}
...
Let $\omega \in \Omega^q(A, E)$.
    ...
    $$
        \omega_i :\left( \Gamma_{U_i}(TM)\oplus C^\infty({U_i}, \alg g) \right) \times \cdots  \left( \Gamma_{U_i}(TM)\oplus C^\infty({U_i}, \alg g) \right) \to C^\infty({U_i}, V).
    $$
...
\end{eqnsplit}
is an isomorphism of differential complexes, over the isomorphism of the graded rings $\Omega^\bullet_U(A)$ and $\Omega^\bullet(TU \oplus (U \times \alg g))$. In particular
\begin{equation}
...
\begin{align*}
    (\hat d_\phi &\omega)_i(X_1 \oplus \eta_1, \dots,  X_{p+1}\oplus \eta_{p+1}) \\ 
      ...
      &+ \sum_{1 \leq k < j \leq p+1} (-1)^{k+j}\beta_i^{-1}\comp\omega(S_i[ X_k \oplus \eta_k, X_j \oplus \eta_j], \cdots, S_i(X_{p+1} \oplus \eta_{p+1}))\\
...
Thanks to Theorem \ref{theoremLocalizationOfEVectorValuedFormsIsomorphismOfDifferentialRespectsD} even if we don't know explicitly the form $\hat \omega \in \Omega^1(TP^k/S^1, P^k \times i\RR/S^1)$, we can still find (the local trivializations of) $\hat d \hat \omega$. Furthermore, we may apply Theorem \ref{theoFormsAreDiffGModule} along with the second part or proposition \ref{propositionTLAFormsGradedSubalgebraAndTMDifferentialGradedSubalgebra} to show that
    \begin{equation*}
        \hat d \hat \omega_S = \hat d (i\omega^\epsilon_{S;\mu}) \wedge dx^\mu + i  \omega^\epsilon_{S;\mu} d(dx^\mu) + \hat d(i \hat \omega^\epsilon_{S;i}) \wedge Im + i\omega^\epsilon_{S;i} dx^\mu \wedge\hat d_{TU_S \times i\RR} Im;
    \end{equation*}
...
$\hat \beta^i_j: \Omega_{U_{ij}}^q(TU_{ij}\times \alg g, U_{ij}\times V) \to \Omega_{U_{ij}}^q(TU_{ij}\times \alg g, U_{ij}\times V)$ is an isomorphism of differential graded complex. Furthermore, $\hat \alpha^i_j: \Omega^q(TU_{ij}\times \alg g) \to \Omega^q(TU_{ij}\times \alg g)$ is an isomorphism of differential graded algebras.
\end{theorem}
\begin{proof}
