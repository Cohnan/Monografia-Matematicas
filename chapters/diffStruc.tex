...
Let $U\subset M$ be open:
    ...
    \item If $U$ is trivializing neighborhood for both $A$ and $E$, $\Omega_U^n(A, E) \cong$ \\
    $( C^\infty(U, \bigwedge^\bullet \alg g^*) \otimes \Omega^\bullet(TU)) \otimes_{\bb K} V$, where $V$ is the typical fiber of $E$ and the Lie algebra $\alg g$ is the typical fiber of the vertical subalgebroid of $A$.
...
\end{align*} similarly, for any $\beta \in C^\infty(M, \bigwedge^s \alg g^*)$:
\begin{align*}
...
Notice that the subalgebra $C^\infty(M, \bigwedge^\bullet \alg g^*)$ \textit{is not closed under the application of the differential}, since any function $f \in C^\infty(M)$ is an element of $C^\infty(M, \bigwedge^0 \alg g^*)$ as well as of $\Omega^0(TM)$, but $\hat d_{TM \times \alg g} = df \in \Omega^1(TM) \not\subset C^\infty(M, \bigwedge^\bullet \alg g^*)$.
...
\begin{eqnsplit}\label{equationDifferentialOfFormDualToBasisLieAlgebra}
    \hat d_{TM \times \alg g} \epsilon(X \oplus \eta, Y \oplus \theta) 
      ...
      &= - \epsilon[\eta, \theta],
\end{eqnsplit}
for arbitrary $X \oplus \eta, Y \oplus \theta \in \Gamma(TM \times \alg g)$; our assumption that $\epsilon: C^\infty(M, \alg g) \to C^\infty(M)$ is constant is only used to write the last equation (notice that the second to last equation shows, again, that the subalgebra $C^\infty(M, \bigwedge^\bullet \alg^*)$ is not closed under the differential). The last equation is $s\epsilon(\eta, theta)$, where $s$ is the Chevalley-Eilenbert differential on $\bigwedge^\bullet \alg g^*$, extended naturally to functions taking values in this last space.
