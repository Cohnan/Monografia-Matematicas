...
As it is usual, to simplify notation we will often say instead that \emph{$A$ is a Lie algebroid over the manifold $M$ with anchor $a$}, and the brackets will be denoted simply by $[\cdot , \cdot]$ for all Lie algebroids.

When talking about $C^r-$differentiable vector bundles, a vector-bundle chart, or simply a chart, will make reference to a local trivialization $\phi: U \times V \to E|_U$ which is $C^r$-differentiable (and a vector bundle isomorphism, by definition of local trivialization).

In general, when talking about vector bundles, say $E$ over $M$, boldface letters will be used to denote sections of the vector bundle, for instance $\sect \mu \in Gamma(E)$, and its value in the fiber $m \in M$ will be denoted both by $\sect \mu(m)$ and by $\sect \mu_m$.  
...
The following family of examples exemplify an intermediate case where a Lie algebroid is a space of tangent directions, but not all those directions are contained.
...
A distribution $\Delta$ on $M$ is a vector subbundle of $TM$; it is called $q$-dimensional if $\Delta$ is a vector bundle of rank $q$.
\end{definition}

Let $\mathcal F = \set{L_\alpha}_{\alpha \in A}$ be a $p$-dimensional foliation of $M$. Each leaf is a regular submanifold of $M$, therefore $T L_\alpha$ is embedded\todo{is it?} in $TM$ and we may see $T L_\alpha$ as a submanifold of $TM$. The (disjoint) union $E = \bigcup_{\alpha \in A} T L_\alpha \subset TM$ can be given a (smooth) vector bundle structure: around $m \in M$ there is a chart $(U, \psi:U \to \RR^n)$ compatible with the foliation; from it we can construct a vector bundle chart $(U \times \RR^n, \phi: U \times \RR^n \to TM|_{U})$ for $TM$ which can be restricted to a chart $(U \times \RR^p, \tilde{\phi}: U \times \RR^p \to TL_{\alpha}|_{U})$ for $E$, where $E|_U = \bigcup_{\alpha: \, U \inter L_\alpha \neq \varnothing} T L_\alpha|_U$. This charts constructed around all $m\in M$ also show that $E$ is embedded in $TM$ as a vector subbundle.
