...
A natural language to talk about principal bundle connections is that of the Atiyah Lie algebroid $TP/G$ of $P$, since:
    ...
    \end{itemize}
The vector bundle $TP/G$ over $M$ is an example of a transitive Lie algebroid over $M$, but this is a whole class of vector bundles with a Lie bracket structure that are, locally, described by $TM \oplus (M \times \alg g)$ and so we may interpret as spaces of generalized directions on $M$. A main component of a Lie algebroid $A$ is the anchor map $a : A \to TM$ which projects the elements of $A$ onto a tangent direction on $M$, and therefore general Lie algebroids, those with non-fiberwise-surjective anchor, allow the flexibility of not forcing all tangent direction on $M$ be ``represented'' in the Lie algebroid. Once this language is adopted, the core concepts of gauge theories mentioned above may be abstracted in the concepts of Lie algebroids, Lie algebroid connections, and representations of Lie algebroids in an ``associated'' vector bundle.
