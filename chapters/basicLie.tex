...
\begin{itemize}
    \item the first map can not be extended to general sections $\mu \in \Gamma(E)$ because it is not, in general, the case that $\cl{\sect \mu(p)} = \cl{\mu(p')}$ even when $p, p' \in \pi^{-1}(m)$;
...
\begin{align} \label{theoAnchorUpsectEqn}
    \pi^G_*(\sectoid X) = \pi_*(\upsect{\sectoid X}) \in \Gamma(TM).
\end{align}
\end{theorem}
...
Let $U \subset$ be open, and $\mathcal U = \pi^{-1}(U) \subset P$. The $C^\infty(U)$-module of local sections $\Gamma_U(Tp/G)$ is isomorphic to the space of loca $G$-invariant sections $\Gamma_{\mathcal U}(TP)$.
\end{theorem}

Based on this theorem, \ptext{we will usually understand $\Gamma^G(TP)$ as the space of sections of $TP/G$, and we might say that a $G$-invariant section $\upsect{\oid X} \in \Gamma^G(TP)$ ``is a section of the Atiyah Lie algebroid $TP/G$''}.
