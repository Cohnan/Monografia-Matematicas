...
Finally, the following proposition allows a description of the sections of the vector bundle $E/G$ from \ref{3.1.1}.

\begin{proposition}
Let $(E, p^E, P)$ be as in \ref{3.1.1}. The space of (smooth) sections of $E/G$ is $C^\infty(M)$-module isomorphic to the $G$-equivariant sections of $E$ 

\[ 
    \Gamma^G(E) = \set{\sect{\mu} \in \Gamma(E) \st \forall p \in P, \forall g \in G:\, \sect \mu(pg) = \sect \mu(p)g}
\] 

where $f \sect \mu (p) := (f \circ \pi)(p) \sect \mu(p)$ for any $\sect \mu \in \Gamma^G(E)$, $p \in P$, via the map $\Gamma(E/G) \to \Gamma^G(E)$, $\sect \mu \mapsto \overline{\sect \mu}$ defined by 
\[
    \overline{\oid {\sect \mu}}(p)
\]
