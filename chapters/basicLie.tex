...
Given $p_0 \in P$, to construct an equivariant $\pi$-saturated chart near $p$ we will start with a chart $\phi:U \times G \to \mathcal U = \pi^{-1}(U)$ for $P$ where $U$ is the range of a chart $\theta:\RR^n \to U$ for $M$. The desired chart $\Phi: \mathcal U \times (\RR^n \oplus \alg g) \to TP|_{\mathcal U}$ for $TP$ will be, basically, $\phi^*$, after we correctly identify 

    \begin{itemize}
    
    \item $T(U)$ with $U \times \RR^n$ via $\theta_*$,  
    
    \item $T(G)$ with $G \times \alg g$ via $R_{g*}$,
    
    \item and, therefore, the vector bundle $T(U \times G) \cong T(U) \boxplus T(G)$ with $\mathcal U \times (\RR^n \oplus \alg g) (U \times \RR^n) \boxplus (G \times \alg g)$ 
