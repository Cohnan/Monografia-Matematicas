...
%     &= - \der{t}[t = 0] \psi_{t*} (\upsect{j(\sect \nu)}_{\psi_{-t}(p)}) \\
...
% \end{proof}
\begin{definition}
 Given $\eta in \alg g$, let \emph{the (right-invariant) vector field on $G$ generated by $\eta$}, denoted by $\sect \eta^L \in \Gamma(G)$, be defined by
\begin{align}
    \sect \eta^L_g := \der{t}[t = 0] \exp{(t \eta)}\, g \in T_g G && g \in G.
\end{align}
