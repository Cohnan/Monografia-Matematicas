...
\begin{enumerate}
    ...
    \item $E$ is covered by $G$-equivariant $\pi$-saturated charts. That is, for each $p_0 \in P$ there is a chart $(U, \psi: \mathcal U \times V \to E|_{\mathcal U})$ of $E$, where $p_0 \in \mathcal U = \pi^{-1}(U)$ for some open $U \subset M$, that satisfies 
    \begin{align*}
        \psi(p, v)g = \psi(pg, v)
    \end{align*}
...
\begin{align} \label{eqnProjectionOfQuotient}
    p^{E/G}(\cl{\mu_p}) = \pi(p^E(\mu_p)) \in M
\end{align}
for any $\mu_p \in E_p$, as indicated by the commutative diagram.
