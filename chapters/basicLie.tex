...
\begin{lemma}\label{lemmaSectionLocal}
The bracket is a local operator. In particular, a unique bracket on local sections induced.
\end{lemma}

\begin{proof}
By section 7 of Tu. The formula for the local bracket is... \complete{just rewrite what's in Tu Ch 7}
\end{proof}

The following theorem gives us insight on the structure of Lie algebroids, specifically on its ``vertical'' part, i.e. the kernel of the anchor map.

\begin{theorem} \label{theoFiberLie}
Let $A$ be a Lie algebroid over the manifold $M$ with anchor $a$, and suppose $ker(a)$ is a vector subbundle of $A$. Then $ker(a)$ is a Lie algebroid over $M$ with trivial anchor, i.e. $ a = 0$, and the bracket on the sections of $ker(a)$ restrict to a Lie bracket on the fibers, making \emph{each fiber of $ker(a)$ a Lie algebra}.
\end{theorem}
...
%\begin{equation}\label{sectoidFormula}
...
\begin{proof}
Constant rank: immersion, then subbundle.

Easy to see that acting with $g$ makes you fall inside again.
\end{proof}
...
\begin{equation}
    \begin{tikzcd}
        ...
        T^\pi P/G \ar[dashed]{r}{j} \arrow{d}{\pi_*^G} & TP/G \arrow{d}{\pi_*^G} \\
        M \arrow{r}{id_M} & M
    \end{tikzcd}
\end{equation}
where the top vectical arrows are the quotient maps, and, abusing notation, $\pi_*^G$ is also used to denote $\pi_*^G|_{T^\pi P/G}$.
\end{theorem}

\begin{proof}
Satisfies conditions of 3.1.1. Exactly the same as $TP$

$i$ satisfies conditions of 3.2: $i$ clearly $G$-equivariant, so $j$ satisfies everything except being embedding.

Momentarily, let $\natural^\pi: T^\pi P \to T^\pi/G$ and $\natural: TP \to TP/G$ be the quotient maps. Then, for any $\ppal X \in T^\pi P$, $\natural^\pi(\ppal X) = \set{\ppal X \, g \st g \in G} = \natural(\ppal X)$, so \[j(\natural^\pi(\ppal X)) = \natural(\ppal X) = \natural^\pi(\ppal X)\], so $j$ is indeed an inclusion.
\end{proof}

\begin{theorem}
The bracket of the Lie algebroid $TP/G$ induces a bracket on $T^\pi P/G$. This makes $(T^\pi P/G, \pi_*^G, M)$ an Atiyah Lie algebroid with trivial anchor.
\end{theorem}

\begin{proof}
All we need to proof is that the bracket of $TP/G$ of sections of $T^\pi P/G$ is again a section of this kind. Let $\sectoid X, \sectoid Y \in \Gamma(T^\pi P/G) \subset \Gamma(TP/G)$, then $\pi_*^G([\sectoid X, \sectoid Y]) = [\pi_*^G (\sectoid X), \pi_*^G(\sectoid Y)] = 0$, so $([\sectoid X, \sectoid Y] \in \Gamma(T^\pi P/G)$.
\end{proof}
...
\begin{theorem}
Let $g \in G$ act to the left on $\alg g$ by $Ad_g: \alg g \to \alg g$, $\eta \mapsto \frac{d}{dt}|_{t=0} g exp(t \eta) g^{-1}$ (derivative at the identity of $AD_g: G \to G$, $h \mapsto g h g^{-1}$). For any $p \in P$ let $rm_p: G \to P$, $g \mapsto gp$. The map 
    \begin{align}
        rm_{p*, 1}: \alg g \to T_p G,& &\eta \mapsto \frac{d}{dt}_{t = 0}p \exp{(t \eta)}
    \end{align}
    induces an injective vector bundle morphism $j: P \times V / G \to TP/G$ such that 
    \begin{equation}
        j(\cl{p, \alg \eta}) = \cl{\frac{d}{dt}_{t = 0} p \exp(t \alg \eta)} \in T^\pi P/G \subset TP/G
    \end{equation} for any $(p, \alg \eta) \in P \times \alg g$. Furtheremore, the vector bundle $P \times \alg g/G$ is isomorphic to $T^\pi P/G$ through $j$.
\end{theorem}
...
\begin{definition}[AtiyahSequence]
JOjo
\end{definition}
