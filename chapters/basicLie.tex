...
So, for $\sect \mu, \sect \nu \in \Gamma_{\mathcal U}^G(E)$, $p \in P$, $g \in G$ arbitrary,
\begin{align*}
    [\sect \mu, \sect \nu]_{pg}(f)
        =& \sect \mu_{pg}(\sect \nu(f)) - \sect \nu_{pg}(\sect \mu (f))\\
...
This bracket inherits the $\RR-linearity$, the Jacobi identity and the alternating property from the bracket in $\Gamma_U(TP/G)$, that is why it is a Lie algebra structure.
