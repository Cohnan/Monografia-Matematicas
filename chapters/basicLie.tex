...
\begin{definition}[LAB]\label{defnLAB}
    ...
    \item $L$ together with a field of Lie brackets $[\cdot, \cdot]:\Gamma(L) \times \Gamma(L) \to \Gamma(L)$ is a \emph{Lie algebra bundle} or \emph{LAB} if $L$ admits an atlas, called \emph{an LAB atlas}, $\set{\phi_i: U_i \times \alg g_i \to L_{U_i}}_{i \in I}$, where $\alg g_i$ is a Lie algebra and each $\phi_{i, m}: \alg g_i \to L_m$ is a Lie algebra isomorphism. An LAB is said to \emph{have fiber type $\alg g$} is there is an LAB atlas such that $\alg g_i = \alg g$ for all $i \in I$.
...
\begin{remark}
Even though the bracket in $ker(A)$, when it is a vector bundle to begin with, gives the structure of a Lie algebra to each of its fibers, $ker(A)$ is not necessarily a LAB since there might not exist an atlas of local trivializations $\phi: U \times \alg g \to ker(A)|_U$ compatible with the brackets.
\end{remark}
...
It has a vector bundle atlas $\set{(U_i, \phi^\oplus_i: U_i \times (\RR^{n_i} \oplus \alg g) \to \pi^{-1}(U_i) )}_{i \in I}$, where $n_i$ is some positive integer and $U_i$ is a chart neighborhood of $M$ that simultaneously trivializes the vector bundles $TM$ and $M \times \alg g$ through the coordinate maps $\phi^{TM}_i: U_i \times \RR^{n_i} \to TU_i$  and $\phi^\alg g = id_{U_i \times \alg g}$.

We now define the main ingredients of the trivial Lie algebroid:
    \begin{itemize}
    \item The anchor $a: TM \oplus (M \times \alg g) \to TM$, $X \oplus \eta \mapsto X$. This map is a vector bundle map as it clearly respects the fibers and, locally on a neighborhood chart, is $\tilde a = (\phi^{TM}_i)^{-1} \circ a \circ \phi^{\oplus}_i: (m, (v, \eta)) \mapsto v$ which is smooth since it is simply a projection, hence $a$ is smooth.
...
Let $(E, p, M)$ be a vector bundle. Its \emph{algebroid of derivations} $\alg D(E)$ is analogous to the Lie algebra of infinitesimal linear transformations $\alg{gl}(V)$ of a vector space $V$. In fact, if $E = M \times V$ is a trivial vector bundle, $\alg D(E)$ is isomorphic to the trivial Lie algebroid $TM \oplus (M \times \alg{gl}(V))$. This Lie algebroid will be important to define a representation of a Lie algebroid on a vector bundle, a generalized notion of associated vector bundle to a principal bundle.

Let $End(E)$ denote the vector bundle whose sections are vector bundle endomorphisms or $E$, which has $End(E_m)$ as fiber at $m \in M$. It is known that vector bundle morphisms over a manifold are in a bijective correspondence with $C^\infty(M)$-maps between the spaces of sections, so we may regard the sections of $End(E)$ as $C^\infty(M)$-linear maps $\Gamma(E) \to \Gamma(E)$, also called \emph{$0$th-order differential operators on $E$}.
...
Let $P(M, G)$ be a principal bundle over $M$ with the finite dimensional Lie group $G$ as fiber which acts on the right by the smooth action $R: G \times P \to G$.

This is the (family of) Lie algebroids that arise naturally in traditional gauge theories, explicitly in the context of principal connections.
