...
For any $p \in P$ and $g \in G$, denote by $m_p: G \to P$, $g \mapsto gp = R_g(p)$. The map 
    \begin{align}
        -m_{p*, 1}: \alg g \to T_p G,& &\eta \mapsto \frac{d}{dt}_{t = 0}p \exp{(-t \eta)} \equiv -\eta^*_p
    \end{align}
...
The vector bundle morphism $j$ induces a $C^\infty(M)$-linear map between sections, but since $\Gamma(P \times g / G) \cong C^\infty_G(P, \alg g)$ and $\Gamma(TP/G) \cong \Gamma^G(TP)$, for $\stilde \eta \in C^\infty_G(P)$ we will also denote by $j$ the induced map:
\begin{eqnsplit}
    j : C^\infty_G(P, \alg g) &\to \Gamma^G(TP)\\
    j(\stilde \eta)(p) &= \frac{d}{dt}_{t = 0} p \exp(-t \stilde \eta(p))
\end{eqnsplit}
