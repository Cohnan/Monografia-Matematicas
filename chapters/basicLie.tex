...
Recall that lemma \ref{lemmaSectionLocal} indicated that allowed us to restrict a Lie algebroid to an open subset $U$ of the base manifold. We now study how a transitive Lie algebroid is a collection of locally defined trivial Lie algebroids correctly pasted together.

\begin{definition}[Lie algebroid atlas]
Let $L \hookrightarrow^j A \twoheadrightarrow TM$ be a transitive Lie algebroid sequence, and let $\alg g$ be a Lie algebra. Given an open cover $\set{U_i}_{i \in I}$ of $M$, a \emph{a Lie algebroid atlas for $A$} is a collection $\set{U_i, \phi_i:U_i \times \alg g \to L_{U_i}, \Theta_i: TU_i \to A_{U_i}}_{i \in I}$ such that:
    \begin{itemize}
    
    \item each $\Theta^i: TU_i \to A_{u_i}$ is Lie algebroid morphisms,
    
    \item the family $\set{\phi_i}$ is a LAB atlas for the adjoint bundle 
    
    \item 
        \begin{align} \label{eqnCompLocal}
            [\Theta_i(\sect X), j\circ \phi_i(\sect \mu)] = j\circ \phi_i(\sect X(\sect \mu))
        \end{align}
    for all $\sect X \in \Gamma(TU_i)$, $\sect \mu \in C^\infty(U_i, \alg g) \cong \Gamma(U_i \times \alg g)$. Such .
    
    \end{itemize}
\end{definition}

\begin{theorem}
Every transitive Lie algebroid is locally trivial.
\end{theorem}
\begin{proof}
...
For a locally trivial transitive Lie algebroid $L \hookrightarrow^j A \twoheadrightarrow TM$ with a Lie algebroid atlas $\set{U_i, \phi_i:U_i \times \alg g\to L_{U_i}, \Theta_i}_{i \in I}$, the maps:
\begin{align}
    S_i:& TU_i \oplus (U_i \times \alg g) &\to& A_{U_i}\\
        & S \oplus \mu & \mapsto & \Theta_i(X) + \phi_i(\mu)
\end{align}
are Lie algebroid isomorphisms thanks to equation (\ref{eqnCompLocal}) which guarantees the compatibility with the bracket. Furthermore, for each $i \in I$ 
\begin{align*}
    \phi_i = S_i^+: U_i \times \alg g \to L_{U_i}
\end{align*} and 
\begin{align*}
    \Theta_i(X) = S_i(X \oplus 0)
\end{align*}
for all $X \in TU_i$.

Now, to understand the transition between charts of a Lie algebroid atlas, let 
\begin{align*}
    a^i_j:& U_{ij} = U_i \inter U_j &\to& Aut(\alg g)\\
          & m & \mapsto & \phi^{-1}_{i, m} \circ \phi_{j, m}
\end{align*} be the transition functions for $\set{\phi_i}$
