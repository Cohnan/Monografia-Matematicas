...
Let $g \in G$ act to the right on the vector bundle $(TP, \pi_*, P)$ by the push-forward of the right translation map $R_g: P \to P$ on $P$. Then, the quotient space $TP/G$ has a (smooth) vector bundle structure over $M$, with projection map $(TP/G, \pi_*^G, M)$, such that the quotient map $\natural: TP \to TP/G$ is a linear surjective submersion such that, for any $\ppal X \in TP$
\begin{align}\label{eqnAnchorAtiyah}
\pi_*^G(\cl{\ppal X}) = \pi_*(\ppal X).    
\end{align}
\end{theorem}
$(TP/G, \pi_*^G, M)$ and $\pi_*^G: TP/G \to TM$ are called \emph{the vector bundle of the Atiyah Lie algebroid} and \emph{the anchor of the Atiyah Lie algebroid associated to the principal bundle $P$}.
...
The following proposition and its corollary allow us to define smooth vector bundle maps that start in quotient vector bundles. In particular, it will allow us to define the anchor $a:TP/G \to TM$ of what will be the Atiyah Lie algebroid, and later on to define the embedding of the adjoint Lie algebroid into $TP/G$.
