...
In an informal manner, we might say that the matter fields are defined $G$-equivariantly on the total space $P$, but the equations of motion involve spacial derivatives over the base manifold $M$; the connection on $P$ is equivalent to a determination of a $G$-invariant notion of horizontallity on $P$, and this horizontal directions are then equated with directions on the base manifold. We may thus say, informally, that the core of a gauge theory consists on:
\begin{enumerate}
...
\ptext{A vector bundle $E$ over a manifold $M$ will usually be denoted by its projection map $p^E:E \to M$ or by the triple $(E, p^E, M)$. A vector-bundle chart, or simply a chart, will make reference to a pair $(U \subset M, \psi: U \times V \to E|_U)$, where $U$ is an open subset of $M$ and $\psi$ is a local trivialization which is $C^\infty$-differentiable (and a vector bundle isomorphism, by definition of local trivialization). Additionally, just in this chapter, for the sake of clarity boldface letters will be used to denote sections of the vector bundle, for instance $\sect \mu \in \Gamma(E)$, and its value in the fiber $m \in M$ will be denoted both by $\sect \mu(m)$ and by $\sect \mu_m$.}\improvement{Cambié esto de lugar}
...
A \emph{Lie algebroid over $M$} is a triple $(q:A \to M, a, [\cdot, \cdot ])$, where $q:A \to M$ is a vector bundle, 
    ...
    \item \emph{the bracket of $A$} $[\cdot, \cdot ]: \Gamma(A) \times \Gamma(A) \to \Gamma(A)$ is a Lie algebra structure on the $\bb K$-vector space $\Gamma(A)$ (i.e. a $\bb K$-bilinear alternating map satisfying the Jacobi identity)
...
\end{proposition}
\begin{proof}
By the alternating property of the bracket, it is enough to prove that if $\sectoid Y_1, \sectoid Y_2 \in \Gamma(A)$ coincide in $U$, then $[\sectoid X, \sectoid Y_1]_m = [\sectoid X, \sectoid Y_2]_m$. By the $\bb K$-linearity of the bracket, we may prove instead that if $\sectoid Y \in \Gamma(A)$ vanishes in $U$, then $[\sectoid X, \sectoid Y]_m = 0$.
...
A \emph{$1$st-order differential operator on $E$} is an $\bb K$-linear map 
\[
    D: \Gamma(E) \to \Gamma(E)
\]
...
This bracket inherits the $\bb K$-linearity, the Jacobi identity and the alternating property from the bracket on $\Gamma_{\mathcal U}(TP)$, meaning that it is a Lie algebra structure on $\Gamma_U(TP/G)$.
...
Throughout the document we will develop some explicit examples of transitive Lie algebroids and of the different structures built on them.  We will do so parting from two big families of principal bundles given a normed division algebra $\FF$ over $\RR$ which is one of $\RR, \CC, \HH$: let $d$ be the dimension of $\FF$ as an $\bb K$-vector space, then for any $n \in \ZZ_{\geq 1}$ the sphere $S^{dn - 1} \subset \FF^n$ on which the Lie group
...
\ptext{The way to describe the Lie algebroid that will be most useful to use will be through their local trivializations as transitive Lie algebroids studied in section \ref{chp:Connection:}, and this requires a local description of both the base manifolds and the principal bundles, which we will now develop.}
...
, as we'll see in \ref{}, the \lbtext{quaternionic Hopf bundle} is the induced $S^3$-principal bundle $S^3 \hookrightarrow S^7 \to S^4$ with projection map $\pi := F^{-1} \circ \tilde \pi: S^7 \to S^4$.
...
It is clear that $\rho$ is $C^\infty(M)$-linear in $\sectoid X$ and $\bb K$-linear in $\sect \mu$. That $\rho(\sectoid X): \Gamma(E) \to \Gamma(E)$ is a derivation, i.e. $\bb K$-linear and satisfies Leibniz, follows once we recall the definition of the anchor in $\Gamma(TP/G)$ in equation (\ref{anchorAtiyah}), which tells us that $a(\sectoid X) = \upsect{\sectoid X}(f \circ \pi)$, therefore:
\begin{multline*}
...
Let $G$ be a group with Lie algebra $\alg g$, and the vector space $V$ be a representation space of $G$. Suppose that $A$ is a transitive Lie algebroid with $\alg g$ as the typical fiber of its vertical part, and let $V$ be a vector space on which $G$ acts. Let $U_i$ and $U_j$, $U_{ij} \neq \emptyset$ be open subsets of $M$ that trivialize $A$. If we allow $TU_i \times \alg g$ and $TU_j \times \alg g$ to be represented in $U_i \times V$ and $U_j \times V$, respectively, via the group representation \eqref{equationGroupInducedRepresentationInTrivialAlgebroids}, we can ask ourselves what conditions need to be satisfied for those representations to be the trivializations of a global representation of $A$ on a vector bundle $E$ that trivializes on $U_i$ and $U_j$. Letting $\beta^i_j = \beta_i^{-1} \comp \beta_j : U_{ij} \times V \to U_{ij} \times V$ be the transition function of $E$ from $U_j$ to $U_i$, a quick analysis shows some conditions are imposed on them: let $\mu$ be a section on $E$ such that $f \in C^\infty(U_{j}, V)$ is its trivialization over $U_i$, i.e. $\beta_i(f) $ be the trivialization of a section $\mu \in \Gamma(E)$, hence $\beta_j(f) = \mu|_{U_j}$, then the compatibility condition 
\begin{equation}
...
\noindent Recall the map $j$ of the Atiyah sequence \ref{}. For any $p \in P$ and $\eta \in \alg g$, the immersion and injective Lie algebroid morphism $j$ is defined by equation (\ref{formulaJ}):
\begin{eqnsplit*}
...
The compatibility condition $[\up{\Theta}_i(X), \up{j \psi(\eta)}] = \up j(\tilde \psi_i(\sect X(\stilde \eta)))$  \todo{HACER}
\end{proof}
...
\begin{proposition}
For any $i, j \in I$ such that $U_i \inter U_j \neq \emptyset$, the change of coordinate maps for the LAB atlas $\{\psi_i: U_i \times \alg g \to \mathcal U_i \times \alg g/G\}$ are
\begin{eqnsplit}
    \alpha^i_j: U_{ij} \times \alg g &\to U_{ij} \times \alg g\\
...
For any $i, j \in I$ such that $U_i \inter U_j \neq \emptyset$, define the $\alg g$-valued forms Maurer-Cartan forms over $U_{ij}$
\begin{eqnsplit}
    \chi^i_j: TU_{ij} \to& U_{ij} \times \alg g \\
        X &\mapsto \psi_i^{-1} \circ l^i_j(X)\\
...
The open $U_i$ trivializes $E$ as $\beta_i: U_i \times V\to E|_{U_i}$
...
\begin{remark}
{For any transitive Lie algebroid sequence $0 \to L \xrightarrow{j} A \xrightarrow{a} \to TM \to 0 $, a complete description of the Lie algebroid is achieved only with knowledge of an open cover $\set{I_i}_{i \in I}$ from a transitive Lie algebroid atlas %$\{U_i, \psi_i:U_i \times \alg g \to L|_{U_i}, \nabla^{0,i}: TU_i \to A|_{U_i}\}_{i \in I}$, 
...
\begin{remark}{For the principal bundles we have been working with, the open cover $\set{U_S, U_N}$ trivializes both the base space $S^n$ and the principal bundles, so \rtext{we get a full description of the transitive Lie algebroids by describing exactly two maps: $\alpha^N_S$ and $\chi^N_S$}}.
\end{remark}
...
\begin{align}\label{equationChangeOfCoordinateOfAlgebroidElementTPkS2}
...
Notice that the local frame \rtext{$\{\partial_\phi, \partial_{\xi^1}, \partial_{\xi^2}\} \in \Gamma_{\mathcal U_{SN}}(TS^3)$ is made of $S^1$-invariant vector fields, making up a local frame of $TS^3/S^1$: 
\[
    \{\underline{\partial_\phi}, \underline{\partial_{\xi^1}}, \underline{\partial_{\xi^2}}\} \in \Gamma_{U_{SN}}(TS^3/S^1)
\] }
...
The following is a intuitive, global description of the Atiyah Lie algebroid $TS^3/S^1$ associated to the complex Hopf bundle, which can only be applied to $U_{SN} = U_S \inter U_N = S^2 - \set{(0, 0, -1), (0, 0, 1)}$. \ptext{We will ignore adding to each map an indication that it is the restriction of the original map to $U_{SN}$}.
